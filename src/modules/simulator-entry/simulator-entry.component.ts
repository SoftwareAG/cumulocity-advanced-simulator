...
import { overlayLabels } from '@models/overlayLabels.const';
...
export class SimulatorEntryComponent implements OnInit, OnDestroy {
  ...
  overlayState = false;
  @ViewChild('test', { static: true }) overlay;
  ...
  editOnClick() {
    ...
    const initialState = {
      labels: { ok: 'upload', cancel: 'Cancel', type: 'upload' },
      modalTitle: 'Modal with component',
      header: this.overlay,
      name: this.testing
    };
    const modal = this.modalService.show(OverlayComponent, { initialState });
