...
export class SimulatorSettingsService {
  ...
  generateInstructions(): CommandQueueEntry[] {
    ...
    this.updateCommandQueueAndIndicesFromIndexedCommandQueue(
  ...
  removeIndices(
    commandQueueEntryWithIndex: IndexedCommandQueueEntry
  ): CommandQueueEntry {
    return (({ deviation, mirrored, index, ...nonIndex }) => nonIndex)(
      commandQueueEntryWithIndex
    );
  }

  removeIndicesFromIndexedCommandQueueArray(
    indexedCommandQueueArray: IndexedCommandQueueEntry[]
  ): CommandQueueEntry[] {
    let commandQueueWithoutIndices = [];
    indexedCommandQueueArray.forEach((entry) =>
      commandQueueWithoutIndices.push(this.removeIndices(entry))
    );
  ...
  generateAlarmsOrEventsOrSleep() {
    if (
      ...
      if (event.messageId === MessageIds.Basic) {
        ...
        event.messageId === MessageIds.LocationUpdate ||
        event.messageId === MessageIds.LocationUpdateDevice
      ) {
        let instruction: Instruction = {
          messageId: event.messageId,
  ...
  pushToResultTemplate(instruction: Instruction) {
    let toBePushed: CommandQueueEntry = this.instructionService.instructionToCommand(
      instruction
    );
    let index = String(this.allInstructionsArray.length - 1);
  ...
  updateCommandQueueAndIndicesFromIndexedCommandQueue(
    indexedCommandQueue: IndexedCommandQueueEntry[]
  ) {
    this.setIndexedCommandQueue(indexedCommandQueue);
    let commandQueue = this.removeIndicesFromIndexedCommandQueueArray(
      indexedCommandQueue
    );
  ...
  objectContainsSearchString(series, searchString) {
    ...
    const value = _.pickBy(modifiedSeries, (value, key) => {
      if (
        ...
        isNaN(Number(searchString.toString()))
      ) {
        return (
          key
            .toString()
  ...
  modifyInstructionSeries(series) {
    let modifiedSeries =
      series.type === "SmartRest"
        ? { ...series.instruction, type: series.type }
        : series;
