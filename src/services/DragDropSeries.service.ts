...
export class DragDropSeriesService {
  ...
  createUpdatedIndexedCommandQueue(indexedCommandQueue, filtered) {
    let rearranged = Object.values(this.helperService.groupBy(indexedCommandQueue, 'index'));
  ...
  createArrayOfSingleInstructions(indexedCommandQueue) {
    let arrayOfSingleInstructions = indexedCommandQueue.filter((entry) => entry.index === 'single');
    let arrayOfSingleInstructionObjects = [];
    arrayOfSingleInstructions.forEach((singleInstruction) => {
      const pos = indexedCommandQueue.findIndex((entry) => _.isEqual(entry, singleInstruction));
